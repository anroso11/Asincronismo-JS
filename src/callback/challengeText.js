...
const API = 'https://api.escuelajs.co/api/v1'; // llamar la API 

//Funcion Principal que obtiene la informacion del producto como un objeto
// dos parametros urlApi , callback-> retorna los datos o error la llamada
function fetchData(urlApi, callback){
    //Instanciar la referencia - inicialización
    let xhttp = new XMLHttpRequest();

    //Abrir una conexion open (tipo: GET o POST )
    ...
    xhttp.onreadystatechange = function(event){
        ...
        if(xhttp.readyState === 4){
            ...
            if(xhttp.status === 200){
                //JSON Transformacion de la informacion para pasarlos en objetos
...
fetchData (`${API}/products`, function(error1,data1){
    ...
    //se invoca nuevamente la función fetchData con el fin de acceder a un objeto puntual del arreglo data1, se envia como parámetros la url de la API apuntando al atributo del primer objeto de arreglo data1 y nuevamente una función anónima.    
        ...
        fetchData(`${API}/categories/${data2?.category?.id}`, function(error3, data3){
            ...
            //Se imprime el nombre de la categoria del producto. 
